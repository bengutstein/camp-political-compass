...
export function CompassChart({ x, y, showTooltip = true }: { x: number; y: number; showTooltip?: boolean }) {
  ...
  return <div className="w-full"><svg viewBox={`0 0 ${size} ${size}`} role="img" aria-label={`Compass chart: your result is x ${x}, y ${y}`} className="h-auto w-full overflow-visible">
    ...
    <text x={pad + 12} y={pad + 22} fontSize="11" fontWeight="700" letterSpacing="1.1" fill="#18211e">{q["upper-left"].name.toUpperCase()}</text><text x={size - pad - 12} y={pad + 22} textAnchor="end" fontSize="11" fontWeight="700" letterSpacing="1.1" fill="#18211e">{q["upper-right"].name.toUpperCase()}</text>
    <text x={pad + 12} y={size - pad - 12} fontSize="11" fontWeight="700" letterSpacing="1.1" fill="#18211e">{q["lower-left"].name.toUpperCase()}</text><text x={size - pad - 12} y={size - pad - 12} textAnchor="end" fontSize="11" fontWeight="700" letterSpacing="1.1" fill="#18211e">{q["lower-right"].name.toUpperCase()}</text>

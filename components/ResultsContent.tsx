...
function SavePublicResult({ name, gesherYear, saveError, saved, saving, onNameChange, onGesherYearChange, onSave }: { name: string; gesherYear: string; saveError: string; saved: boolean; saving: boolean; onNameChange: (value: string) => void; onGesherYearChange: (value: string) => void; onSave: () => void }) {
  return <div className="mt-8 max-w-sm border-t rule pt-7 lg:mt-10"><label className="block text-sm font-bold">Save this result publicly<input value={name} onChange={(event) => onNameChange(event.target.value)} placeholder="Enter your display name" maxLength={60} className="mt-2 min-h-12 w-full border border-line bg-white px-4 py-3 text-base font-normal outline-none transition focus:border-moss focus:ring-2 focus:ring-moss/30 sm:text-sm" /></label><label className="mt-4 block text-sm font-bold">Gesher Year<input value={gesherYear} onChange={(event) => onGesherYearChange(event.target.value)} type="number" inputMode="numeric" min="1900" max="2100" placeholder="e.g. 2024" required className="mt-2 min-h-12 w-full border border-line bg-white px-4 py-3 text-base font-normal outline-none transition focus:border-moss focus:ring-2 focus:ring-moss/30 sm:text-sm" /></label><p className="mt-2 text-xs leading-5 text-ink/60">Your name, Gesher Year, and compass position will appear on the public map.</p>{saveError && <p className="mt-3 text-sm font-bold text-clay" role="alert">{saveError}</p>}<button type="button" onClick={onSave} disabled={!name.trim() || !gesherYear || saving} className="mt-4 min-h-12 bg-moss px-5 py-3 text-sm font-bold text-white transition hover:bg-ink disabled:cursor-not-allowed disabled:opacity-40">{saving ? "Saving…" : saved ? "Saved to the map" : "Save my result"}</button></div>;
}

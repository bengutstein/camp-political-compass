...
export function CompassPreview({ points, featuredPoint, highlightedPointId }: { points?: readonly CompassPoint[]; featuredPoint?: CompassPoint; highlightedPointId?: string | null }) {
  ...
  return <div className="relative mx-auto aspect-square w-full max-w-[420px] overflow-hidden border border-ink/20 bg-white shadow-[8px_8px_0_0_#d9e4db]" role="group" aria-label="Interactive four-quadrant compass chart">
    ...
    <span className="absolute left-4 top-4 text-[10px] font-bold uppercase tracking-[.08em] text-red-700">{q["upper-left"].name}</span><span className="absolute right-4 top-4 text-right text-[10px] font-bold uppercase tracking-[.08em] text-red-700">{q["upper-right"].name}</span><span className="absolute bottom-4 left-4 text-[10px] font-bold uppercase tracking-[.08em] text-red-700">{q["lower-left"].name}</span><span className="absolute bottom-4 right-4 text-right text-[10px] font-bold uppercase tracking-[.08em] text-red-700">{q["lower-right"].name}</span>

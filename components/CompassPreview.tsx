...
export function CompassPreview({ points, featuredPoint }: { points?: readonly CompassPoint[]; featuredPoint?: CompassPoint }) {
  ...
  return <div className="relative mx-auto aspect-square w-full max-w-[420px] overflow-hidden border border-ink/20 bg-white shadow-[8px_8px_0_0_#d9e4db]" aria-label="Preview of a four-quadrant compass chart">
    ...
    {displayedPoints.map((result) => {
      ...
      return <div key={pointKey} onMouseEnter={() => setHoveredPoint(pointKey)} onMouseLeave={() => setHoveredPoint(null)} className="group absolute z-10 group-hover:z-30 group-focus-within:z-30" style={{ left: `${((result.x + 10) / 20) * 100}%`, top: `${((10 - result.y) / 20) * 100}%` }}><div className={`h-3.5 w-3.5 -translate-x-1/2 -translate-y-1/2 rounded-full transition-all group-hover:scale-125 ${isDimmed ? "bg-[#9ca3af] opacity-35 shadow-none" : "bg-ink shadow-sm"}`} /><span className={`pointer-events-none absolute left-2 top-1 inline-block origin-left whitespace-nowrap text-[9px] font-bold transition-colors ${isDimmed ? "text-[#9ca3af]" : "text-ink"} ${result.alwaysShowName ? "opacity-100 transition-transform group-hover:scale-110" : "opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100"}`}>{result.name}</span></div>;
    })}
    {featuredPoint && <div onMouseEnter={() => setHoveredPoint(featuredPoint.id ?? featuredPoint.name)} onMouseLeave={() => setHoveredPoint(null)} className="group absolute z-20 group-hover:z-30" style={{ left: `${((featuredPoint.x + 10) / 20) * 100}%`, top: `${((10 - featuredPoint.y) / 20) * 100}%` }}><div className={`h-5 w-5 -translate-x-1/2 -translate-y-1/2 rounded-full transition-all group-hover:scale-110 ${hoveredPoint !== null && hoveredPoint !== (featuredPoint.id ?? featuredPoint.name) ? "bg-[#9ca3af] opacity-35 shadow-none ring-0" : "bg-red-600 shadow-md ring-2 ring-red-600/40"}`} /><span className={`pointer-events-none absolute left-3 top-1 whitespace-nowrap text-[10px] font-extrabold transition-colors ${hoveredPoint !== null && hoveredPoint !== (featuredPoint.id ?? featuredPoint.name) ? "text-[#9ca3af]" : "text-ink"}`}>{featuredPoint.name}</span></div>}
  </div>;
}

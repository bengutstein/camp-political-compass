...
export function CompassPreview({ points }: { points?: readonly CompassPoint[] }) {
  ...
  return <div className="relative mx-auto aspect-square w-full max-w-[420px] overflow-hidden border border-ink/20 bg-white shadow-[8px_8px_0_0_#d9e4db]" aria-label="Preview of a four-quadrant compass chart">
    ...
    {displayedPoints.map((result) => <div key={result.id ?? result.name} className="group absolute z-10" style={{ left: `${((result.x + 10) / 20) * 100}%`, top: `${((10 - result.y) / 20) * 100}%` }}><div className="h-3.5 w-3.5 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white bg-ink shadow-sm transition-transform group-hover:scale-125" /><span className={`pointer-events-none absolute left-2 top-1 whitespace-nowrap text-[9px] font-bold text-ink transition-opacity ${result.alwaysShowName ? "opacity-100" : "opacity-0 group-hover:opacity-100 group-focus-within:opacity-100"}`}>{result.name}</span></div>)}
  </div>;
}

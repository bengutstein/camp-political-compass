...
export function CompassPreview({ points, featuredPoint, highlightedPointId }: { points?: readonly CompassPoint[]; featuredPoint?: CompassPoint; highlightedPointId?: string | null }) {
  ...
  return <div className="relative mx-auto aspect-square w-full max-w-[420px] overflow-hidden border border-ink/20 bg-white shadow-[8px_8px_0_0_#d9e4db]" role="group" aria-label="Interactive four-quadrant compass chart">
    ...
    {displayedPoints.map((result) => {
      ...
      return <div key={pointKey} role="button" tabIndex={0} aria-label={`Show ${result.name}'s result`} onPointerEnter={() => setHoveredPoint(pointKey)} onPointerLeave={() => setHoveredPoint(null)} onClick={() => setHoveredPoint(isActive ? null : pointKey)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setHoveredPoint(isActive ? null : pointKey); } }} className={`absolute h-11 w-11 -translate-x-1/2 -translate-y-1/2 cursor-pointer rounded-full ${isActive || isHighlighted ? "z-30" : result.alwaysShowName ? "z-20" : "z-10"}`} style={{ left: xPosition(result.x), top: yPosition(result.y) }}><span aria-hidden className={`absolute left-1/2 top-1/2 h-[10.5px] w-[10.5px] -translate-x-1/2 -translate-y-1/2 rounded-full transition-all ${isActive ? "scale-125" : ""} ${isHighlighted ? "ring-2 ring-moss ring-offset-2" : ""} ${isMuted ? "bg-[#9ca3af] opacity-35 shadow-none" : "bg-ink shadow-sm"}`} /><span className={`pointer-events-none absolute top-1/2 -translate-y-1/2 whitespace-nowrap text-[11px] font-bold transition-all ${labelOnLeft ? "right-[calc(50%+7px)] origin-right text-right" : "left-[calc(50%+7px)] origin-left"} ${isMuted ? "text-[#9ca3af]" : result.alwaysShowName ? "text-red-700" : "text-ink"} ${showLabel ? "opacity-100" : "opacity-0"} ${result.alwaysShowName && isActive ? "scale-110" : ""}`}>{result.name}</span></div>;
    })}

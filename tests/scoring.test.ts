...
describe("calculateScore", () => {
  it("normalizes all strongly agree responses into the expected range", () => { const score = calculateScore(answers(2), questions); expect(score.x).toBe(3); expect(score.y).toBe(5); });
  it("normalizes all strongly disagree responses", () => { const score = calculateScore(answers(-2), questions); expect(score.x).toBe(-3); expect(score.y).toBe(-5); });
  it("combines mixed answers and reverse scoring", () => { const score = calculateScore([{ questionId: "x", value: 2 }, { questionId: "y", value: -2 }, { questionId: "both", value: 1 }, { questionId: "reverse", value: 2 }], questions); expect(score.x).toBe(1.5); expect(score.y).toBe(-10); expect(score.quadrant).toBe("lower-right"); });
  ...
  it("keeps a neutral-ish combination near center", () => { const score = calculateScore([{ questionId: "x", value: 1 }, { questionId: "y", value: -1 }, { questionId: "both", value: -1 }, { questionId: "reverse", value: -1 }], questions); expect(score.x).toBe(4.5); expect(score.y).toBe(-2.5); });
